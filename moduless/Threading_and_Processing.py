    ...
    def download_images(self):
        try:
            ...
            for i, img_tag in enumerate(img_tags):
                image_name = self._download_image(img_tag)
                progress = int((i+1) / self.total_images * 100)
                print(f"Putting progress value of {progress} from download_images at iteration {i}")
            ...
            while count <= self.total_images:
                if self.image_queue:
                    ...
                    if image.split('.')[0].isdigit():
                        ...
                        file_name = f_lst[0]
                        file_extension = f_lst[-1]
                        new_name = str(count-minus).zfill(3)
                        convert_image_format('', self.cache_folder, file_name, file_extension, new_name=new_name, target_format="png")
                        print(f'Renamed {image} to {new_name + ".png"}')
                    else: minus += 1
                    progress = int((count) / self.total_images * 100)
                    print(f"Putting progress value of {progress} from process_images at count {count}")
                    self.process_progress_queue.put(progress)
                    if count == self.total_images: count += 1
            return True
        except:
            return False
    ...
    def cache_current_chapter(self):
        ...
        while True:
            if not download_thread.is_alive() and not process_thread.is_alive() and (self.download_progress_queue.empty() and self.process_progress_queue.empty()):
                break
            print("Download Thread alive: " + str(download_thread.is_alive()), "Process Thread alive: " + str(process_thread.is_alive()))
...
# Pool-threading, Connection-Pooling and multi-processing in line
from aplustools.webtools import Search
from aplustools.imagetools import OnlineImage, OfflineImage
save_image = OnlineImage.save_image
convert_image_format = OnlineImage.convert_image_format
    ...
    def download_images(self):
        try:
            ...
            with ThreadPoolExecutor() as executor:
                image_names = list(executor.map(self._download_image, img_tags))
            ...
            with Pool(processes=4) as pool:
                while count <= self.total_images:
                    if not self.image_queue.empty():
                        ...
                        if None in [file_name, file_extension, new_name]: continue
...
# Pool-threading, Connection-Pooling and multi-processing concurrently
from aplustools.webtools import Search
from aplustools.imagetools import OnlineImage, OfflineImage
save_image = OnlineImage.save_image
convert_image_format = OnlineImage.convert_image_format
