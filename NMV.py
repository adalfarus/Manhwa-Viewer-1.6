...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.system = System()
        self.theme = self.system.get_windows_theme() or os.environ.get('MV_THEME') or "light"
        self.update_theme(self.theme.lower())
        ...
        new = not os.path.isfile(db_path)
        db = database.DBManager(db_path)

        self.settings = Settings(new, db, {"geometry": "100, 100, 800, 630"})
    ...
    def setup_gui(self):
        ...
        # Scroll Area
        self.scrollarea = QAdvancedSmoothScrollingArea(sensitivity=self.settings.get_scrolling_sensitivity())
        ...
        # advanced_settings_button.clicked.connect(self.export_settings)  # Menu
        # export_settings_button.clicked.connect(self.export_chapter)  # Menu
    ...
    def selected_chosen_result(self, new_title):
    ...
    def get_wanted_width(self):
        scroll_area_width = self.scrollarea.viewport().width()
